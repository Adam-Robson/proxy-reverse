...
describe('rewritePath', () => {
	...
	it('replacePath overrides pathname entirely', () => {
		expect(rewritePath('/api/users', { replacePath: '/v2/users' })).toBe('/v2/users');
	});

	it('replacePath takes priority over stripPrefix and addPrefix', () => {
		expect(
			rewritePath('/api/users', { replacePath: '/static', stripPrefix: '/api', addPrefix: '/v2' }),
		).toBe('/static');
	});
	...
	it('stripPrefix is a no-op when prefix does not match', () => {
		expect(rewritePath('/other/users', { stripPrefix: '/api' })).toBe('/other/users');
	});
