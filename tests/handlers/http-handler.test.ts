...
describe('HttpHandler', () => {
	...
	it('returns 502 when upstream connection is refused', async () => {
		...
		const config: ConfigType = {
			port: 0,
			routes: [{ match: '/', upstreams: [{ host: '127.0.0.1', port: closedPort }] }],
		};
		const handler = new HttpHandler(config);
		const srv = await startServer((req, res) => handler.handle(req, res));
	...
	it('returns 502 when upstream times out', async () => {
		const hanging = await new Promise<{ server: net.Server; port: number }>((resolve) => {
		...
		const config: ConfigType = {
			port: 0,
			routes: [{ match: '/', upstreams: [{ host: '127.0.0.1', port: hanging.port }], timeout: 100 }],
		};
		const handler = new HttpHandler(config);
		const srv = await startServer((req, res) => handler.handle(req, res));
		const { status } = await request(srv.port, '/test');
	...
	it('returns 413 when content-length exceeds maxBodySize', async () => {
		...
		const { status } = await new Promise<{ status: number }>((resolve, reject) => {
			const req = http.request(
				{ host: '127.0.0.1', port: srv.port, path: '/', method: 'POST', headers: { 'content-length': '200' } },
	...
	it('handles concurrent requests without errors', async () => {
		const results = await Promise.all(
			Array.from({ length: 10 }, (_, i) => request(proxy.port, `/api/concurrent-${i}`)),
		);
