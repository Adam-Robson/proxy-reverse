...
function startTcpUpstream(onConnect: (socket: net.Socket) => void): Promise<{ server: net.Server; port: number }> {
...
function startHttpProxy(handler: HttpHandler): Promise<{ server: http.Server; port: number }> {
	return new Promise((resolve) => {
...
describe('handleWebSocketUpgrade', () => {
	...
	it('tunnels the upgrade request to the upstream and returns 101', async () => {
		...
		const cfg: ConfigType = {
			port: 0,
			routes: [{ match: '/ws', upstreams: [{ host: '127.0.0.1', port: upstream.port }] }],
		};
